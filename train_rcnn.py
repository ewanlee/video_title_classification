...
def assign_pretrained_word_embedding(
        sess, vocab, vocab_size, textrcnn,
        word2vec_model_path=\
                'data/news_12g_baidubaike_20g_novel_90g_embedding_64.model'):
...
def do_eval(sess, textrcnn, X_val, y_val, batch_size):
    ...
    for start, end in zip(
        ...
        feed_dict = {textrcnn.input_x: X_val[start:end], textrcnn.dropout_keep_prob: 1}
        if not FLAGS.multi_label_flag:
            feed_dict[textrcnn.input_y] = y_val[start:end]
        else:
            feed_dict[textrcnn.input_y_multilabel] = y_val[start:end]
        curr_eval_loss, logits, curr_eval_acc = sess.run(
                [textrcnn.loss_val, textrcnn.logits, textrcnn.accuracy], feed_dict)
