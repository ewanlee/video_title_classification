...
tf.app.flags.DEFINE_string("ckpt_dir", "text_rnn_title_desc_checkpoint/", "checkpoint location for the model")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        # Instantiate Model
        textrnn = TextRNN(FLAGS.num_classes,
        ...
        if os.path.exists(FLAGS.ckpt_dir + 'checkpoint'):
            ...
            if FLAGS.use_embedding:
                assign_pretrained_word_embedding(sess, vocab, vocab_size, textrnn)
        ...
        for epoch in range(curr_epoch, FLAGS.num_epochs):
            ...
            for start, end in zip(
                ...
                feed_dict = {
                        textrnn.input_x: X_train[start:end], textrnn.dropout_keep_prob: 0.5}
                if not FLAGS.multi_label_flag:
                    feed_dict[textrnn.input_y] = y_train[start:end]
                else:
                    feed_dict[textrnn.input_y_multilabel] = y_train[start:end]
                curr_loss, curr_acc, _ = sess.run(
                        [textrnn.loss_val, textrnn.accuracy, textrnn.train_op], feed_dict)
            ...
            if epoch % FLAGS.validate_every == 0:
                eval_loss, eval_acc = do_eval(sess, textrnn, X_val, y_val, batch_size)
        ...
        # report result
        test_loss, test_acc = do_eval(sess, textrnn, X_val, y_val, batch_size)

def assign_pretrained_word_embedding(
        sess, vocab, vocab_size, textrnn,
...
def do_eval(sess, textrnn, X_val, y_val, batch_size):
    ...
    for start, end in zip(
        ...
        feed_dict = {textrnn.input_x: X_val[start:end], textrnn.dropout_keep_prob: 1}
        if not FLAGS.multi_label_flag:
            feed_dict[textrnn.input_y] = y_val[start:end]
        else:
            feed_dict[textrnn.input_y_multilabel] = y_val[start:end]
        curr_eval_loss, logits, curr_eval_acc = sess.run(
                [textrnn.loss_val, textrnn.logits, textrnn.accuracy], feed_dict)
