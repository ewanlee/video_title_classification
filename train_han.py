...
tf.app.flags.DEFINE_string("ckpt_dir", "han_title_desc_checkpoint/", "checkpoint location for the model")
...
tf.app.flags.DEFINE_integer("hidden_size", 64, "hidden size")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        han = HierarchicalAttention(
                ...
                FLAGS.is_training, multi_label_flag=False)
        # Initialize save
        ...
        if os.path.exists(FLAGS.ckpt_dir + 'checkpoint'):
            ...
            if FLAGS.use_embedding:
                assign_pretrained_word_embedding(sess, vocab, vocab_size, han)
        curr_epoch = sess.run(han.epoch_step)

        # feed data and training
        number_of_training_data = len(X_train)
        batch_size = FLAGS.batch_size
        previous_eval_loss =1e9
        best_eval_loss =1e9
        for epoch in range(curr_epoch, FLAGS.num_epochs):
            ...
            for start, end in zip(
                ...
                feed_dict = {
                        han.input_x: X_train[start:end], han.dropout_keep_prob: 0.5}
                if not FLAGS.multi_label_flag:
                    feed_dict[han.input_y] = y_train[start:end]
                else:
                    feed_dict[han.input_y_multilabel] = y_train[start:end]
                curr_loss, curr_acc, _ = sess.run(
                        [han.loss_val, han.accuracy, han.train_op], feed_dict)
            ...
            sess.run(han.epoch_increment)

            # validation
            if epoch % FLAGS.validate_every == 0:
                eval_loss, eval_acc = do_eval(sess, han, X_val, y_val, batch_size)
                ...
                if eval_loss > previous_eval_loss:
                    ...
                    learning_rate1 = sess.run(han.learning_rate)
                    lrr = sess.run([han.learning_rate_decay_half_op])
                    learning_rate2 = sess.run(han.learning_rate)
                    print("learning_rate1:", learning_rate1, " ;learning_rate2:", learning_rate2)
                else:
                    if eval_loss < best_eval_loss:
                        print("going to save the model.eval_loss:",eval_loss,";best_eval_loss:",best_eval_loss)
        ...
        # report result
        test_loss, test_acc = do_eval(sess, han, X_val, y_val, batch_size)

def assign_pretrained_word_embedding(
        sess, vocab, vocab_size, han,
    ...
    t_assign_embedding = tf.assign(han.Embedding, word_embedding)
...
def do_eval(sess, han, X_val, y_val, batch_size):
    ...
    for start, end in zip(
        ...
        feed_dict = {han.input_x: X_val[start:end], han.dropout_keep_prob: 1}
        if not FLAGS.multi_label_flag:
            feed_dict[han.input_y] = y_val[start:end]
        else:
            feed_dict[han.input_y_multilabel] = y_val[start:end]
        curr_eval_loss, logits, curr_eval_acc = sess.run(
                [han.loss_val, han.logits, han.accuracy], feed_dict)
